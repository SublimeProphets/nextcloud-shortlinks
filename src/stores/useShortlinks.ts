...
const state = reactive({ links: [] as ShortLink[], folders: [] as Folder[], tags: [] as Tag[], loading: false, error: '', page: 1, hasMore: false, search: '', system: 'all', folderId: null as number | null, tagIds: [] as number[], selected: new Set<number>() })
...
async function refresh(): Promise<void> {
	...
	try {
		const [links, folders, tags] = await Promise.all([api.listLinks({ page: state.page, perPage: 50, search: state.search, system: state.system, folderId: state.folderId ?? undefined, tagIds: state.tagIds }), api.listFolders(), api.listTags()])
...
async function loadMore(): Promise<void> {
	...
	try {
		...
		const result = await api.listLinks({ page: next, perPage: 50, search: state.search, system: state.system, folderId: state.folderId ?? undefined, tagIds: state.tagIds })
