...
final class LinkPageService {
	...
	/** @return array{page:array<string,mixed>,links:list<array<string,mixed>>,files:list<array<string,mixed>>,contacts:list<array<string,mixed>>,owner:string} */
	...
	private function apply(LinkPage $page, array $data): void {
		...
		foreach (['theme' => 'setThemeJson', 'header' => 'setHeaderJson', 'footer' => 'setFooterJson'] as $key => $setter) {
			if (array_key_exists($key, $data)) {
				$value = is_array($data[$key]) ? array_slice($data[$key], 0, 30) : [];
				$page->$setter(json_encode($value, JSON_THROW_ON_ERROR));
			}
		}
	}
	...
	private function selectedLinks(LinkPage $page): array {
		$folderIds = $this->decodeIds($page->getFolderIds());
		$tagIds = $this->decodeIds($page->getTagIds());
		$explicit = $this->decodeIds($page->getLinkIds());
