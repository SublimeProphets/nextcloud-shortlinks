...
import { mdiAccountOutline, mdiArrowLeft, mdiCheck, mdiClose, mdiContentSaveOutline, mdiEyeOutline, mdiFileMultipleOutline, mdiFilePlusOutline, mdiFormatListBulleted, mdiLockOutline, mdiMagnify, mdiPaletteOutline, mdiShapeOutline } from '@mdi/js'
...
import type { Folder, LinkPage, LinkPageDraft, PageAccessMode, PageContact, PageLayout, ShortLink, Tag } from '../types'
...
async function pickFiles() {
	try {
		const picker = getFilePickerBuilder(t('shortlinks', 'Choose files for this page'))
			...
			.addButton({
				label: t('shortlinks', 'Add selected files'),
				variant: 'primary',
				callback: () => {},
			})
			.build()
			...
			<div class="page-editor__controls">
				...
				<div v-else-if="tab === 'content'" class="editor-panel">
					<section>
						...
						<h2>{{ t('shortlinks', 'Individual links') }}</h2><p>{{ t('shortlinks', 'Pin individual links in addition to automatic sources.') }}</p><div class="source-list source-list--links">
							<label v-for="link in links" :key="link.id"><input type="checkbox" :checked="draft.linkIds.includes(link.id)" @change="toggleId('linkIds', link.id)"><span><strong>{{ link.title || link.slug }}</strong><small>…/{{ link.slug }}</small></span></label><p v-if="loadingLinks">
				...
				<div v-else class="editor-panel">
					<section>
						...
						</div><label class="select-field">{{ t('shortlinks', 'Group links by') }}<select v-model="draft.grouping"><option value="none">{{ t('shortlinks', 'No grouping') }}</option><option value="folder">{{ t('shortlinks', 'Folder') }}</option><option value="tag">{{ t('shortlinks', 'Tag') }}</option></select></label>
					</section><section>
						...
						<h2>{{ t('shortlinks', 'Colors') }}</h2><div class="color-fields">
						...
						<h2>{{ t('shortlinks', 'Header and footer') }}</h2><div class="field-toggles">
							...
							</NcCheckboxRadioSwitch><NcCheckboxRadioSwitch v-model="draft.footer.enabled" type="switch">
